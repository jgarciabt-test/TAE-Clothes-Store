...
$result = mysql_query("SELECT cat_id, cat_main_cat, cat_name, picture.pic_url, MIN(prod_price) FROM TAE_STORE.categories JOIN TAE_STORE.products ON(cat_id = prod_cat) JOIN TAE_STORE.picture ON(cat_pic = pic_id) WHERE prod_state = 1 GROUP BY cat_id");
...
if(mysql_num_rows($result) > 0){
	...
	while($row = mysql_fetch_array($result)){
		...
		$element["cat_main_cat"] = $row["cat_main_cat"];
